...
namespace DC.Api.Filter
{
    ...
    public class ApiFilter : ActionFilterAttribute
    {
        ...
        public override void OnActionExecuting(ActionExecutingContext context)
        {
            ...
            if (Action != "Login" && Action!= "QueryProductH5")
            {
                if (context.HttpContext.Request.Headers.ContainsKey("Authorization"))
                {
                    var tokenHeader = context.HttpContext.Request.Headers["Authorization"];
                    if (!string.IsNullOrEmpty(tokenHeader) && tokenHeader != "Bearer")
                    {
                        tokenHeader = tokenHeader.ToString().Substring("Bearer ".Length).Trim();

                        var _tokenService = AutofacHelper.GetScopeService<ITokenService>();
                        TokenModel tm = _tokenService.SerializeToken(tokenHeader);
                        UserModel.Account = tm.Account;
                        UserModel.Token = tokenHeader;
                        userID = tm.Id;
                    }
                }
            }
