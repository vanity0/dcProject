<template>
  <div>
    ...
    <params
      ref="params"
      v-show="!showPermission && !modifyEntity.showChilder"
    ...
    >
      ...
      <span slot="action" slot-scope="text, item">
        <a-space>
          <a @click="handleDetails(item)" v-action:dcUser_details> <a-icon type="eye" />详情 </a>
          <a @click="handleEdit(item)" v-action:dcUser_edit><a-icon type="edit" />修改</a>
          <a @click="handleDelete(item)" v-action:dcUser_delete><a-icon type="delete" />删除</a>
          <a-dropdown v-if="$route.meta.showMore">
            <a-menu slot="overlay">
              <a-menu-item>
                <a @click="handleOpenChilder(item)" v-action:dcUser_childer><a-icon type="plus-circle" />下级员工</a>
              </a-menu-item>
              <a-menu-item>
                <a @click="handleResetPwd(item)" v-action:dcUser_resetPwd><a-icon type="key" />重置密码</a>
              </a-menu-item>
              <a-menu-item>
                <a @click="handlePermission(item)"><a-icon type="snippets" />权限分配</a>
              </a-menu-item>
            </a-menu>
            <a>
              更多
      ...
      <template slot="title">
        <a-row>
          <a-col :span="24">
            ...
            <a-space>
              <a-button type="primary" icon="plus-circle" v-action:dcUser_create @click="handleAdd">添加</a-button>
              <a-button type="danger" icon="minus-circle" v-action:dcUser_delete @click="handleBatchDelete"
                >删除</a-button
              >
            </a-space>
          </a-col>
    ...
    <a-modal centered title="修改密码" :visible="visible" @cancel="handleCancel">
      <a-spin :spinning="spinning">
        <a-input v-model="newPwd" placeholder="请输入新密码" />
      </a-spin>
      <template slot="footer">
        <a-button :loading="spinning" @click="handleSaveNewPwd"> 确认修改 </a-button>
      </template>
    </a-modal>
    ...
    <!--下级员工页面-->
    <childer
      v-if="modifyEntity.showChilder"
      @goBack="goBack"
      @handleRefresh="handleRefresh"
      :modifyEntity="modifyEntity"
    />

    <permission v-if="showPermission" @handleHidePermission="handleHidePermission" :roleId="roleId" />
  </div>
</template>
<script>
...
export default {
  name: 'dcUser',
  components: { params, childer, modify, myTable, permission },
  data() {
    return {
      modifyEntity: {
        // 是否展示子集页面
  ...
  methods: {
    ...
    handleSaveNewPwd() {
      if (this.newPwd.length <= 0) {
        ...
        var that = this
        this.$confirm({
          title: '确定要修改密码为：' + that.newPwd + '吗?',
          onOk() {
            that.spinning = true
            resetPwd({ id: that.id, newPwd: that.newPwd })
              .then((res) => {
                if (res.success) {
                  that.visible = false
    ...
    //#region  下级员工
    handleOpenChilder(item) {
      this.modifyEntity = {
        parentRoleId: item.roleId,
    ...
    goBack() {
      this.modifyEntity.showChilder = false
    },
    // 打开添加页面
    handleAdd() {
      this.modifyEntity = {
