  ...
  <a-modal centered width="80%" title="推广数据" :visible="visible" @cancel="handleCancel">
    ...
    >
       <span slot="action" slot-scope="text, item">
        <a-space>
          <a v-if="item.status === '待更新'" @click="handleSetStatus(item, '审核通过')" v-action:product_registAuditSuccess style="color: green"
            ><a-icon type="smile" />审核通过</a
          >
          <a v-if="item.status === '待更新'" @click="handleSetStatus(item, '拒绝通过')" v-action:product_registAuditFail style="color: red"
      ...
      <template slot="title">
        <a-row type="flex" justify="end">
          <a-col :span="24">
            <a-space>
              <a-button type="danger" icon="minus-circle" @click="handleBatchDelete" v-action:product_registDelete
                >删除</a-button
              >
    ...
    loadData() {
      ...
      this.$nextTick(() => {
        ...
        let queryParams = {
          ...
          current: that.$refs.myTable.pagination.current,
          pageSize: that.$refs.myTable.pagination.pageSize,
        }
