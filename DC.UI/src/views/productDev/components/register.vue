    ...
    >
      <template slot="title">
        <a-row type="flex" justify="end">
          <a-col :span="24">
            <a-space>
              <a-button type="danger" icon="minus-circle" @click="handleBatchDelete" v-action:productDev_deleteRegister
                >删除</a-button
              >
      ...
      <span slot="action" slot-scope="text, item">
        <a
          ...
          v-action:productDev_deleteRegister
          :disabled="item.Status === '待更新'"
        >
...
import { registerDevColumns, registerDev2Columns, queryRegister, deleteRegister } from '@/api/register'
export default {
  name: 'register',
  components: { modify, registerParams },
  props: {
    // 产品ID
    productId: {
      type: String,
  ...
  mounted() {
    this.columns = this.$store.getters.roles[0].role === '一级推手' ? registerDevColumns : registerDev2Columns
    this.loadData()
  },
  methods: {
    //#region  返回
